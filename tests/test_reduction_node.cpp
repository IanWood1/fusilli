...
TEST_CASE("ReductionNode getName correctly propagates the attribute name",
  ...
  Context ctx;
  ReductionAttr attr;
  attr.setName("foo_reduction");

  ReductionNode node(std::move(attr), ctx);
  REQUIRE(node.getName() == "foo_reduction");
}
...
TEST_CASE("ReductionNode with SUM mode spatial reduction", "[reduction_node]") {
  Context ctx;
  ...
  y = node.reductionAttr.getY();
  REQUIRE(y != nullptr);
  REQUIRE(y->getDim() == std::vector<int64_t>{n, c, 1, 1});
  REQUIRE(y->getDataType() == DataType::Float);
}

TEST_CASE("ReductionNode with MIN mode channel reduction", "[reduction_node]") {
  Context ctx;
  ctx.setIODataType(DataType::Float);
  ReductionAttr attr;
  attr.setMode(ReductionAttr::Mode::MIN);
  ...
  auto y = std::make_shared<TensorAttr>();
  y->setDim({n, 1, h, w}).setStride({h * w, h * w, w, 1});
  ...
  y = node.reductionAttr.getY();
  REQUIRE(y != nullptr);
  REQUIRE(y->getDim() == std::vector<int64_t>{n, 1, h, w});
  REQUIRE(y->getDataType() == DataType::Float);
}
...
TEST_CASE("ReductionNode with MAX mode full tensor reduction",
  ...
  y = node.reductionAttr.getY();
  REQUIRE(y != nullptr);
  REQUIRE(y->getDim() == std::vector<int64_t>{1, 1, 1, 1});
  REQUIRE(y->getDataType() == DataType::Float);
}
