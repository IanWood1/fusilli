...
TEST_CASE("ReductionAttr all modes", "[reduction_attr]") {
  ReductionAttr attr;

  SECTION("SUM mode") {
    attr.setMode(ReductionAttr::Mode::SUM);
    REQUIRE(attr.getMode() == ReductionAttr::Mode::SUM);
    REQUIRE(ReductionAttr::kModeToStr.at(ReductionAttr::Mode::SUM) == "SUM");
  }

  SECTION("MIN mode") {
    attr.setMode(ReductionAttr::Mode::MIN);
    REQUIRE(attr.getMode() == ReductionAttr::Mode::MIN);
    REQUIRE(ReductionAttr::kModeToStr.at(ReductionAttr::Mode::MIN) == "MIN");
  }

  SECTION("MAX mode") {
    attr.setMode(ReductionAttr::Mode::MAX);
    REQUIRE(attr.getMode() == ReductionAttr::Mode::MAX);
    REQUIRE(ReductionAttr::kModeToStr.at(ReductionAttr::Mode::MAX) == "MAX");
  }
}
